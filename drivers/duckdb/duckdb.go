// Package duckdb implements the sq driver for SQLite.
// The backing SQL driver is marcboeker/go-duckdb
package duckdb
...
import (
	...
	_ "github.com/marcboeker/go-duckdb" // Import for side effect of loading the driver
...
// See: https://github.com/mattn/go-duckdb#connection-string.
...
func (d *driveri) DriverMetadata() driver.Metadata {
	return driver.Metadata{
		...
		Description: "SQLite",
		Doc:         "https://github.com/marcboeker/go-duckdb",
		IsSQL:       true,
	}
}
...
func (d *driveri) doOpen(ctx context.Context, src *source.Source) (*sql.DB, error) {
	...
	if err != nil {
		return nil, errz.Wrapf(errw(err), "failed to open duckdb source with DSN: %s", fp)
	}
...
func (d *driveri) CopyTable(ctx context.Context, db sqlz.DB,
	...
	ogSchema, ogTbl, err := "","",nil // sqlparser.ExtractTableIdentFromCreateTableStmt(ogTblCreateStmt, false)
	if err != nil {
		return 0, errw(err)
	}
...
func (d *driveri) CatalogExists(_ context.Context, _ sqlz.DB, _ string) (bool, error) {
	return false, errz.New("duckdb: catalog mechanism not supported")
}
...
func (d *driveri) CurrentCatalog(_ context.Context, _ sqlz.DB) (string, error) {
	return "", errz.New("duckdb: catalog mechanism not supported")
}
...
func (d *driveri) ListCatalogs(_ context.Context, _ sqlz.DB) ([]string, error) {
	return nil, errz.New("duckdb: catalog mechanism not supported")
}
...
func NewScratchSource(ctx context.Context, fpath string) (src *source.Source, clnup func() error, err error) {
	...
	clnup = func() error {
		if journal := filepath.Join(fpath, ".db-journal"); ioz.FileAccessible(journal) {
			lg.WarnIfError(log, "Delete duckdb db journal file", os.Remove(journal))
		}

		log.Debug("Delete duckdb scratchdb file", lga.Src, src, lga.Path, fpath)
		if err := os.Remove(fpath); err != nil {
			log.Warn("Delete duckdb scratchdb file", lga.Err, err)
			return errz.Err(err)
		}
...
// which should have the "duckdb://" prefix.
func PathFromLocation(src *source.Source) (string, error) {
	...
	if !strings.HasPrefix(src.Location, Prefix) {
		return "", errz.Errorf("duckdb source location must begin with {%s} but was: %s", Prefix, src.RedactedLocation())
	}

	loc := strings.TrimPrefix(src.Location, Prefix)
	if len(loc) < 2 {
		return "", errz.Errorf("duckdb source location is too short: %s", src.RedactedLocation())
	}
...
//
//	duckdb:///path/to/sakila.db	--> duckdb:///path/to/sakila.db
//	duckdb:sakila.db 						--> duckdb:///current/working/dir/sakila.db
//	duckdb:/sakila.db 						--> duckdb:///sakila.db
//	duckdb:./sakila.db 					--> duckdb:///current/working/dir/sakila.db
//	duckdb:sakila.db 						--> duckdb:///current/working/dir/sakila.db
//
