...
func TestKindFromDBTypeName(t *testing.T) {
	...
	for dbTypeName, wantKind := range testCases {
		gotKind := duckdb.KindFromDBTypeName(ctx, "col", dbTypeName, nil)
...
func TestGetTblRowCounts(t *testing.T) {
	...
	counts, err := duckdb.GetTblRowCounts(th.Context, db, tblNames)
	require.NoError(t, err)
...
func BenchmarkGetTblRowCounts(b *testing.B) {
	...
	testCases := []struct {
		...
		{name: "getTblRowCounts", fn: duckdb.GetTblRowCounts},
	}
