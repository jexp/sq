...
//	"drivers/sqlite3/internal/sqlparser"
...
func (d *driveri) AlterTableColumnKinds(ctx context.Context, db sqlz.DB,
	...
	if len(colNames) != len(kinds) {
		return errz.New("duckdb: alter table: mismatched count of columns and kinds")
	}
	...
	if err = db.QueryRowContext(ctx, q, tblName).Scan(&ogDDL); err != nil {
		return errz.Wrapf(err, "duckdb: alter table: failed to read original DDL")
	}

/*
	allColDefs, err := (nil,nil) // sqlparser.ExtractCreateTableStmtColDefs(ogDDL)
	if err != nil {
		return errz.Wrapf(err, "duckdb: alter table: failed to extract column definitions from DDL")
	}
	...
	for i, colName := range colNames {
		...
		if len(colDefs) != i+1 {
			return errz.Errorf("duckdb: alter table: column {%s} not found in table DDL", colName)
		}
	}
...
*/
	...
	if _, err = db.ExecContext(ctx, nuDDL); err != nil {
		return errz.Wrapf(err, "duckdb: alter table: failed to create temporary table")
	}
	...
	if _, err = db.ExecContext(ctx, copyStmt); err != nil {
		return errz.Wrapf(err, "duckdb: alter table: failed to copy data to temporary table")
	}

	// Drop old table
	if _, err = db.ExecContext(ctx, "DROP TABLE "+stringz.DoubleQuote(tblName)); err != nil {
		return errz.Wrapf(err, "duckdb: alter table: failed to drop original table")
	}
	...
	if _, err = db.ExecContext(ctx, fmt.Sprintf(
		...
		return errz.Wrapf(err, "duckdb: alter table: failed to rename temporary table")
	}
...
func pragmaDisableForeignKeys(ctx context.Context, db sqlz.DB) (restore func(), err error) {
	...
	if err != nil {
		return nil, errz.Wrapf(err, "duckdb: alter table: failed to read foreign_keys pragma")
	}

	if _, err = db.ExecContext(ctx, "PRAGMA foreign_keys=off"); err != nil {
		return nil, errz.Wrapf(err, "duckdb: alter table: failed to disable foreign_keys pragma")
	}

	return func() {
		_, restoreErr := db.ExecContext(ctx, fmt.Sprintf("PRAGMA foreign_keys=%v", pragmaFkExisting))
		if restoreErr != nil {
			lg.FromContext(ctx).Error("duckdb: alter table: failed to restore foreign_keys pragma", lga.Err, restoreErr)
		}
	}, nil
}
